...
const MemberSinglePhoto = props => {
	const [liked, setLiked] = useState(false)
	const [imgIsLoading, setImgIsLoading] = useState(false)
	const { selectedMember, selectedPhoto } = useContext(MemberContext)
		...
		<ImageBackground source={{ uri: selectedPhoto.photo }} style={styles.bgImg} resizeMode={'cover'} blurRadius={6} opacity={0.8}>
			<View style={styles.content}>
				...
				<View style={styles.userInfo}>
					<Image source={{ uri: selectedMember.image }} style={styles.profilePic} />
					<Text style={[styles.username, styles.text]}>{selectedMember.username}</Text>
				</View>
...
export default MemberSinglePhoto
