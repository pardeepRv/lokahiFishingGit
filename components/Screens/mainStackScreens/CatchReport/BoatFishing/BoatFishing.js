	...
	const renderImage = img => {
		switch (img) {
			case 0:
				return (
					<Image
						source={require('../../../../../media/images/catch-report-icons/OffshoreFishing.png')}
						style={styles.scrollIcon}
						onPress={() => {
							alert('Pressed fishtype')
						}}
					/>
				)
			case 1:
				// return <View style={[styles.scrollIcon, styles.bottomIcon]}></View>;
				return <Image source={require('../../../../../media/images/catch-report-icons/BottomFishing.png')} style={styles.scrollIcon} />
			case 2:
	...
	renderContent = data => {
		return (
			<TouchableOpacity
				...
				onPress={() => {
					if (data.item.link === 'OffshoreFishType') {
						setBoatFishingType('Offshore Fishing')
					}
					if (data.item.link === 'BottomFishing') {
						setBoatFishingType('Bottom Fishing')
					}
		...
		<View style={styles.bg}>
			...
			<View style={styles.content}>
				<Carousel
					...
					data={[
						{ image: 0, subtext: 'Offshore Fishing', link: 'OffshoreFishType' },
						{ image: 1, subtext: 'Bottom Fishing', link: 'BottomFishing' },
