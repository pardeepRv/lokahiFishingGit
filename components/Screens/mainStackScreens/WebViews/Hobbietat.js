...
const Hobbietat = ({ navigation }) => {
	const [webPage, setWebPage] = useState('https://www.hawaiifishingtackle.com/');
	const [prevWebPage, setPrevWebPage] = useState(null);
	const webEl = useRef(null);

	useEffect(() => {
        setWebPage('https://www.hawaiifishingtackle.com/')
    }, []);
