...
const ArcSolutions = ({ navigation }) => {
	const [webPage, setWebPage] = useState('https://www.e-arc.com/location/Honolulu/');
	const [prevWebPage, setPrevWebPage] = useState(null);
	const webEl = useRef(null);

	useEffect(() => {
        setWebPage('https://www.e-arc.com/location/Honolulu/')
    }, []);
