...
const hanapaafishing = ({ navigation }) => {
	const [webPage, setWebPage] = useState('https://hanapaafishing.com/');
	const [prevWebPage, setPrevWebPage] = useState(null);
	const webEl = useRef(null);

	useEffect(() => {
        setWebPage('https://hanapaafishing.com/')
    }, []);
