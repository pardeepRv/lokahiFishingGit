...
import { View, Text, TouchableOpacity, SafeAreaView, StyleSheet, Dimensions, ImageBackground, FlatList, ActivityIndicator } from 'react-native'
...
const Tournament = ({ navigation }) => {
	...
	useEffect(() => {
		...
		firestore()
			...
			.then(res => {
				...
				setTournamentItems(tournamentList)
			})
	}, [])
	...
	const tournamentItemsMapped = tournamentItems.map(el => {
		return <TournamentItem key={el.id} item={el} />;
	});
				...
				<View style={styles.content}>
					{tournamentItems.length !== 0 ? (
						<FlatList
							...
							renderItem={({ item }) => <TournamentItem key={item.id} item={item} />}
						/>
					) : (
